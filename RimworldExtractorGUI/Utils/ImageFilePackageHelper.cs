...
namespace RimworldExtractorGUI.Utils
{
    internal static class ImageFilePackageHelper
    {
        ...
        {
            // 2. 메모리 폭발 방지를 위해 FileStream을 사용하여 파이프라이닝 (청크 단위 복사)
            ...
            if (!string.IsNullOrEmpty(imagePath) && File.Exists(imagePath))
            {
                ...
                imgStream.CopyTo(destStream); // Stream.CopyTo를 사용하면 메모리를 거의 먹지 않습니다.
            }
            else
            {
                destStream.Write(DefaultImageBytes, 0, DefaultImageBytes.Length);
            }
